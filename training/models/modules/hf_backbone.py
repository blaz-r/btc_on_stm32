...
from models.modules.dinov3 import Dinov3Backbone
from models.modules.hf_m2f import handle_m2f_swinb_citysem
from models.modules.hf_swin_backbone import HFSwinBackbone
...
class HFBackbone(BaseCDEncoder):
        ...
        if "upernet" in model_name:
            ...
            if self.pretrained:
                ...
                self.backbone = Mask2FormerForUniversalSegmentation.from_pretrained(
                    ...
                    ignore_mismatched_sizes=True,  # ignore in case of returning less than 4 layers
            ...
            try:
                ...
                self.backbone = TimmBackbone(timm_config)
            # TODO: fix this if you do pretraining with timm backbones
            self.patch_size = None
            ...
            if "patch_size" in hf_config:
                ...
                self.patch_size = None
        elif "Dinov2" in hf_config.architectures[0]:
            hf_config.out_features = out_list
            if self.drop_path_rate is not None:
                hf_config.drop_path_rate = self.drop_path_rate
            ...
            self.patch_size = hf_config.patch_size
        elif "DINOv3" in hf_config.architectures[0]:
            if self.drop_path_rate is not None:
                hf_config.drop_path_rate = self.drop_path_rate

            self.backbone = Dinov3Backbone(model_name, hf_config, self.return_layers)
        else:
    ...
    def init_backbone_pt(self):
        """Monkey-patch business :D"""
        # init given backbone
        self.init_backbone()

        if "swin" not in self.model_name:
            return

        # get backbone config from config
        model_name = self.model_name
        hf_config = AutoConfig.from_pretrained(model_name)
        if "ImageClassification" in hf_config.architectures[0]:
            backbone_config = hf_config
        else:
            backbone_config = hf_config.backbone_config

        out_list = [f"stage{i}" for i in self.return_layers]
        backbone_config.out_features = out_list

        # make our version of swin backbone from used config (with mask token and mask forward pass)
        backbone = HFSwinBackbone.from_config(config=backbone_config)
        # copy parameters
        missing, unexpected = backbone.load_state_dict(
            self.backbone.state_dict(), strict=False
        )
        print(f"Loaded backbone into masked version. Missing keys: {missing}")
        if unexpected:
            raise RuntimeError(f"Unexpected keys in backbone state_dict {unexpected}")
    ...
    def forward(self, x, mask=None, hidden3=False):
        if not self.finetune:
            self.backbone.eval()

        if mask is None:
            feat = self.backbone(x, output_hidden_states=hidden3)
        else:
            if len(mask.size()) == 3:
                mask = mask.flatten(1)
            feat = self.backbone(x, mask, output_hidden_states=hidden3)

        rd = {k: v for k, v in zip(self.return_layers, feat.feature_maps)}

        if hidden3:
            rd["h3"] = feat.hidden_states[3]
        return rd

    def get_parameters(self, pretraining: bool) -> list[dict]:
        # TODO - layerwise decay
        if self.finetune:
    ...
    def transfer_from_pretrained(self, state_dict: dict):
        ...
        if "backbone.embeddings.mask_token" in state_dict:
