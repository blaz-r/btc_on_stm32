...
from models.modules.dinov2 import DinoV2Encoder, build_dinov2
from models.modules.hf_backbone import HFBackbone, build_hfbackbone

import importlib


def build_encoder(config, pretraining):
    model_type = config.encoder.class_path.split(".")[-1]

    if model_type == DinoV2Encoder.__name__:
        return build_dinov2(config)
    elif model_type == HFBackbone.__name__:
        return build_hfbackbone(config, pretraining=pretraining)
    else:
        raise ValueError(f"Model type {model_type} not supported")


def build_diff(config, dims, resolutions, pretraining, enc):
...
def build_pre_diff(config, dims, resolutions, pretraining, enc):
    if config.pre_diff is None:
        return None
    split = config.pre_diff.class_path.split(".")
    module_path, class_name = ".".join(split[:-1]), split[-1]

    pre_diff: BaseCDDiff = getattr(importlib.import_module(module_path), class_name)
...
def build_decoder(config, input_sizes, encoder_strides, pretraining, enc):
    config = copy.deepcopy(config)
    if pretraining:
        if "pretrain" in config.decoder and config.decoder.pretrain is not None:
