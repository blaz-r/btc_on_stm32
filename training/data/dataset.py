...
BANDS = [
    ...
    "B09",  # Water vapour
    "B10",  # SWIR - Cirrus B10 not in S2A
    "B11",  # SWIR
    "B12",  # SWIR
]
...
BANDNAME_2_BANDS = {
    "oscd96_ms-rgb": [BANDS.index(b) for b in ["B04", "B03", "B02"]],  # RGB
    "rgb2bgr": [2, 1, 0],  # reindex RGB to BGR
    "oscd96_ms-10": [
        BANDS.index(b)
        for b in ["B04", "B03", "B02", "B05", "B06", "B07", "B08", "B8A", "B11", "B12"]
    ],  # all 10m to 20m
    "oscd96_ms_BEN1": [
        BANDS.index(b)
        for b in ["B02", "B03", "B04", "B08", "B05", "B06", "B07", "B11", "B12", "B8A"]
    ],
    "oscd96_ms_BEN2": [
        BANDS.index(b)
...
class CDDataset(Dataset):
    def __init__(
        self, data: str | list[str], transform, bands, use_hf=True, load_in_mem="direct"
    ):
    ...
    def load_from_hdf5(self, path):
        ...
        reindex = self.bands != "rgb"

        if reindex:
            band_idx = BANDNAME_2_BANDS[self.bands]
            imageA = imageA[..., band_idx]
            imageB = imageB[..., band_idx]
