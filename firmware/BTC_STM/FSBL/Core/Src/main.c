...
/* Early boot diagnostic: blink both STM32N6570-DK user LEDs in the FSBL. */
#define FSBL_LED_BLINK_TEST      0U
#define USER_LED_GREEN_PORT      GPIOO
#define USER_LED_GREEN_PIN       GPIO_PIN_1
#define USER_LED_RED_PORT        GPIOG
#define USER_LED_RED_PIN         GPIO_PIN_10
#define FSBL_LED_BLINK_PERIOD_MS 500U
...
#if FSBL_LED_BLINK_TEST
  GPIO_InitTypeDef user_led_gpio = {0};

  /* LD1 is active high; LD2 is active low. Start with both LEDs off. */
  __HAL_RCC_GPIOO_CLK_ENABLE();
  __HAL_RCC_GPIOG_CLK_ENABLE();
  HAL_GPIO_WritePin(USER_LED_GREEN_PORT, USER_LED_GREEN_PIN, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(USER_LED_RED_PORT, USER_LED_RED_PIN, GPIO_PIN_SET);

  user_led_gpio.Pin = USER_LED_GREEN_PIN;
  user_led_gpio.Mode = GPIO_MODE_OUTPUT_PP;
  user_led_gpio.Pull = GPIO_NOPULL;
  user_led_gpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(USER_LED_GREEN_PORT, &user_led_gpio);

  user_led_gpio.Pin = USER_LED_RED_PIN;
  HAL_GPIO_Init(USER_LED_RED_PORT, &user_led_gpio);

  while (1)
  {
    HAL_GPIO_TogglePin(USER_LED_GREEN_PORT, USER_LED_GREEN_PIN);
    HAL_GPIO_TogglePin(USER_LED_RED_PORT, USER_LED_RED_PIN);
    HAL_Delay(FSBL_LED_BLINK_PERIOD_MS);
  }
#endif
